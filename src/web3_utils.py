...
class Web3Manager:
    ...
    def get_gas_price(self) -> int:
        # use network gas price if possible, else fallback to 20 gwei
        try:
...
def push_root_to_chain(root_hex: str) -> dict:
    ...
    try:
        ...
        tx = {
            ...
            "value": 0,
            "gas": 200000,
            "gasPrice": web3_manager.get_gas_price(),
        ...
        try:
            ...
            # persist anchor to anchors table (best-effort)
            try:
                ...
                with get_conn() as conn:
                    ...
                    cur.execute(
                        ...
                        (root_hex, tx_hash_hex, blk, int(time.time()))
                    )
            except Exception as e:
                print("[push_root_to_chain] DB write failed (anchor):", e)
